...
# получение данных, по которым будет проводится сортировка при парсинге
str111 = "/Users/mariibykova/Desktop/snils.txt"
infile1234 = open(str111, 'r+')
lines12345 = infile1234.readlines()
ft = ""
snils = []
key = 0 # номер столбца со снилсами, устанавливается отдельно для каждого ВУЗа
for i in lines12345:
   ft = i.strip()
   ft = ft.replace(" ", "")
   ft = ft.replace("-", "")
   snils.append(ft.strip())
...
def parser12():
   ...
   data = "po_snils_"+data # редактируем название
   ...
   lines = infile.readlines()
   i =0
   os.chdir(adres) # переход в диреркторию
...
def parser(name, url, adres, key):
   ...
   for table1 in soup.find_all('table',): # пока на странице есть таблицы
      ...
      for j in table1.find_all('tr')[1:]: # строка находится внутри тега <tr
         ...
         # если строка непустая проверяем снилс в данных
         if (len(row)>=key+1):
            for i in range(len(row[key])):
               if (row[key][i] in "0123456789"):
                  str1234 += str(row[key][i])
            str1234 = str1234.replace(" ", "")
            # если снилс есть, то сохраняем строку таблицы в фрейм
            if (str1234 in snils): 
               rows.append(row)
           #length = len(mydata)
      ...
      mydata.to_csv(s, index=False) # экспорт данные в CSV-файл
...
window.title('Parser snils') # вызов функции парсеринга
...
def on_closing(): # сообщение при закрытии программы
